...
output_file = "DenseNet201.csv"

# Load the DenseNet201 model and remove the final classification layer
base_model = DenseNet201(weights='imagenet', include_top=False , pooling='max')
...
def extract_features(img_path):
    ...
    # Preprocess the image using the DenseNet201 preprocess_input function
    x = np.expand_dims(img, axis=0)
    x = preprocess_input(x)

    # Extract features from the image using the DenseNet201 model
